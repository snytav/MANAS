...
/* ****************** BEAM **************************************** */
	for (j = 1; j <= jmb; j++) 
	{
	    ...
	    ux[j - 1] = vb0 + rimp;
	    uy[j - 1] = rnd_gaussian(0.0, Tb*rimp,0);
	    uz[j - 1] = rnd_gaussian(0.0, Tb*rimp,0);
#ifdef DEBUG_INITIAL_PARTICLE_PRINTS	    
	    printf("%10d %25.15e %25.15e %25.15e \n",j,ux[j - 1],uy[j - 1],uz[j - 1]);
#endif	    
...
// 	    ub[j-1] = ubt;
	    ...
	    wb[j - 1] = uz[j - 1] / vb0;
#ifdef DEBUG_INITIAL_PARTICLE_PRINTS	    
	    printf("beam %10d %25.15e %25.15e %25.15e %25.15e %25.15e \n",j,ux[j - 1],vb0,ub[j-1],vb[j - 1],wb[j - 1]);
#endif	    
	}
...
#ifdef DEBUG_INITIAL_PARTICLE_PRINTS	    
	  printf("%10d %25.15e %25.15e %25.15e \n",2*j-2,uf[2*j-2],vf[2*j-2],wf[2*j-2]);
	  printf("%10d %25.15e %25.15e %25.15e \n",2*j-1,uf[2*j-1],vf[2*j-1],wf[2*j-1]);
#endif	  
    }
		...
		double ni,double rbd,int lp)
{
	...
	electrons->m      = -ni/lp;
	ions->m           =  2.0*ni/lp*(1.0+rbd);
	beam_electrons->m = electrons->m*rbd*2.0;

	electrons->m      = -1.0;
	ions->m           =  1.0/1836.0;
	beam_electrons->m = -1.0;
